...
Two readings matter. First, **"native cloud" (rung 3) genuinely answers the fear**: with one DO per vault, a vault instance's code can only reach its own object's storage — cross-tenant exposure requires an ID-routing bug at the narrow addressing layer or a Cloudflare runtime flaw, not any bug anywhere in our 45k LOC; the bulk of the attack surface moves to a platform whose isolation is its core business. (Whether rung 3's isolate model outranks rung 2's hardware VM is debatable — they're different boundaries; both are categorically above rungs 0–1.) Second, **rung 1 gets most of the way there on our own boxes**: per-tenant processes under per-tenant OS users means a vault-code bug exposes only that tenant's files — and it doubles as the blast-radius/noisy-neighbor fix, and benefits self-host multi-user (friends-hosting) too. What is *not* acceptable is shipping strangers on rung 0.
